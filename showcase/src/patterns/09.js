...
//custom hook for clapState change useClapState
...
const ClapContainer = ({ children, setRef, updateClapState, ...restProps }) => {
  return (
    <button ref={setRef} onClick={updateClapState} {...restProps}>
      {children}
    </button>
  );
};
...
const ClapCount = ({ clapCount, setRef, ...restProps }) => {
  return (
...
const Usage = () => {
  const { clapState, getCountProps, getTogglerProps, clapTotalProps, reset } =
